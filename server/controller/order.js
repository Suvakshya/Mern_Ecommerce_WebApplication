...
if (!stripe) throw new Error("Stripe initialization failed - check your API key");
...
export const newOrderOnline = TryCatch(async (req, res) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: lineItems,
    mode: "payment",
    success_url: `${process.env.FRONTEND_URL || process.env.Frontend_Url}/ordersuccess?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${process.env.FRONTEND_URL || process.env.Frontend_Url}/cart`,
    metadata: {
      userId: req.user._id.toString(),
      method,
      phone,
